...
interface EditReminderModalProps {
    ...
    onClose: () => void
    onUpdated: (updatedReminder: ReminderEntry) => void
}
...
function EditReminderModal({ reminder, onClose, onUpdated }: EditReminderModalProps) {
    const initialType = isFinanceEntryType(reminder.type)
        ? reminder.type
        : FINANCE_ENTRY_TYPES.expense
    const initialCategory = getFinanceCategories(initialType).includes(reminder.category as FinanceCategory)
        ? (reminder.category as FinanceCategory)
        : getDefaultFinanceCategory(initialType)

    const [type, setType] = useState<FinanceEntryType>(initialType)
    const [category, setCategory] = useState<FinanceCategory>(initialCategory)
    const [title, setTitle] = useState(reminder.title)
    const [dueDate, setDueDate] = useState<string | null>(normalizeDateKey(reminder.dueDate))
    const [isDatePickerOpen, setIsDatePickerOpen] = useState(false)
    const [amount, setAmount] = useState(String(reminder.amount ?? 0))
    const [notes, setNotes] = useState(String(reminder.notes ?? ""))
    const [errorMessage, setErrorMessage] = useState("")
    const [isSubmitting, setIsSubmitting] = useState(false)

    useEffect(() => {
        const nextType = isFinanceEntryType(reminder.type)
            ? reminder.type
            : FINANCE_ENTRY_TYPES.expense
        const nextCategory = getFinanceCategories(nextType).includes(reminder.category as FinanceCategory)
            ? (reminder.category as FinanceCategory)
            : getDefaultFinanceCategory(nextType)

        setType(nextType)
        setCategory(nextCategory)
        setTitle(reminder.title)
        setDueDate(normalizeDateKey(reminder.dueDate))
        setAmount(String(reminder.amount ?? 0))
        setNotes(String(reminder.notes ?? ""))
    }, [reminder])
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const updatedReminder = await updateReminder(reminder._id, {
            ...
            setErrorMessage(
                ...
                error?.message ||
                "Unable to update the reminder right now."
            )
        } finally {
            ...
            <div className={styles.overlay} role="presentation" onClick={onClose}>
                ...
                <div className={styles.header}>
                    <div className={styles.headerCopy}>
                        <h2 id="edit-reminder-title" className={styles.title}>Edit Reminder</h2>
                        <p className={styles.subtitle}>
                            Update your reminder details. This will affect due-date alerts and upcoming notifications.
                        </p>
                    </div>
                </div>
